    ...
    private IEnumerator SendUnityPostReq(RequestData req, AppsflyerRequestType REQ_TYPE)
    {
        ...
        switch (REQ_TYPE)
        {
            case AppsflyerRequestType.FIRST_OPEN_REQUEST:
                url = isSandbox
                    ? "https://sandbox-events.appsflyer.com/v1.0/c2s/first_open/app/xbox/"
                        + appid
                    : "https://events.appsflyer.com/v1.0/c2s/first_open/app/xbox/" + appid;
                break;
            case AppsflyerRequestType.SESSION_REQUEST:
                url = isSandbox
                    ? "https://sandbox-events.appsflyer.com/v1.0/c2s/session/app/xbox/"
                        + appid
                    : "https://events.appsflyer.com/v1.0/c2s/session/app/xbox/" + appid;
                break;
            case AppsflyerRequestType.INAPP_EVENT_REQUEST:
                url = isSandbox
                    ? "https://sandbox-events.appsflyer.com/v1.0/c2s/inapp/app/xbox/" + appid
                    : "https://events.appsflyer.com/v1.0/c2s/inapp/app/xbox/" + appid;
                break;
            default:
                url = null;
                break;
